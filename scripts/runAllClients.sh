...
updatesFile=${14}
firstProxyPort=${15}
sourceSelectionStrategy=${16}
action=${17} # one of justReplicate justExecute all
queriesToExecute=${18}
publicEndpointPort=${19}
publicEndpointProxyPort=${20}
setupFolder=${21}
federation=${22}
tmpFilePEP=`mktemp`
...
for i in  `seq 0 $last`; do
    ...
    if [ -f "$f" ]; then
        ...
        if [ "$action" != "justReplicate" ]; then
            #./produceAnswer.sh "${l[i]}" "$queriesFile" "$federation" "${firstProxyPort}" "$answersFolder"
	    ./getAnswersSingleEndpoint.sh "${l[i]}" "$queriesFile" "$federation" "$setupFolder" "$publicEndpointPort" "$firstPort" "$answersFolder"
        fi
    fi
done
      ...
      for ss in $sss; do
        if [ "$strategy" = "FEDERATION" ] || [ "$strategy" = "PUBLIC" ]; then
          if [ "$action" != "justReplicate" ] && [ "$action" != "justSelect" ]; then
            ...
            #address=`./getHost.sh $setupFolder/hosts 3040 ${firstProxyPort}`
	    address=localhost
            cd $lilachome/proxy
            java -cp .:$httpcomponentsClientPath/* SingleEndpointProxy2 $address ${publicEndpointPort} ${publicEndpointProxyPort} > $tmpFilePEP &
            pidPEProxy=$!
            cd $p
          fi
        fi
	    ...
	    ./runClient.sh $strategy $queriesFile "${l[i]}" $port $ldfServer ${federationFile} $availability $answersFolder $publicEndpoint $engine $configFile $federation $anapsidFederationFile ${updatesFile}${port} ${proxyPort} ${action} ${address} ${setupFolder}/output${label}${numberClients}Client$port ${graph} > ${tmpFile}
            pid=`cat $tmpFile`
            spids="$spids $pid"
        done
        ...
        if [ "$strategy" = "FEDERATION" ]; then
          for i in `seq 0 $last`; do
            ...
            #proxyAddress=`$lilachome/scripts/getHost.sh $setupFolder/hosts $port $firstPort`
