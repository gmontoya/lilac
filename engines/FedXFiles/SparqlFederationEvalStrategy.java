...
public class SparqlFederationEvalStrategy extends FederationEvalStrategy {
	...
	public CloseableIteration<BindingSet, QueryEvaluationException> evaluateBoundJoinStatementPattern(
		...
		Boolean isEvaluated = false;
                ...
                if (stmt instanceof StatementPattern) { 
		    preparedQuery = QueryStringUtil.selectQueryStringBoundUnion((StatementPattern)stmt, bindings, filterExpr, isEvaluated);
                } else if (stmt instanceof ExclusiveGroup) {
                    preparedQuery = QueryStringUtil.selectQueryStringBoundUnion(((ExclusiveGroup)stmt).getStatements(), bindings, filterExpr, isEvaluated);
                }
	        //System.out.println("SparqlFederationEvalStrategy:l87. preparedQuery: "+preparedQuery);	
	...
	public CloseableIteration<BindingSet, QueryEvaluationException> evaluateGroupedCheck(
		...
		if (bindings.size()==1)
			return stmt.evaluate(bindings.get(0));
                String preparedQuery = null;
	        if (stmt.isExclusiveGroupStatement()) {
                    preparedQuery = QueryStringUtil.selectQueryStringBoundCheck(stmt.getStatementPatterns(), bindings);
                } else {
		    preparedQuery = QueryStringUtil.selectQueryStringBoundCheck(stmt.getStatementPattern(), bindings);
                }		
		CloseableIteration<BindingSet, QueryEvaluationException> result = evaluateAtStatementSources(preparedQuery, stmt.getStatementSources(), stmt.getQueryInfo());
